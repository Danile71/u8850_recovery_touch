...
#define BOARD_HAS_NO_SELECT_BUTTON
#define BOARD_TOUCH_RECOVERY
...
#include "../../vendor/huawei/u8850/recovery/touch.c"
#endif
...
{
        key_queue[key_queue_len++] = KEY_SEARCH;
...
static int usb_connected() {
   /* int fd = open("/sys/class/android_usb/android0/enable", O_RDONLY);
    if (fd < 0) {
        printf("failed to open /sys/class/android_usb/android0/enable: %s\n",
               strerror(errno));
        return 0;
    }
    ...
    if (close(fd) < 0) {
        printf("failed to close /sys/class/android_usb/android0/enable: %s\n",
               strerror(errno));
    }*/
    return 0;
}
